...
// functiontool.New. Namespace — if any — must already be present in the
// context before the tool is invoked; callers set it with postera.WithNamespace
// wherever fits their architecture. It flows to each tool naturally through
// Go's standard context chain.
package adk
...
func Tools(ts *agent.ToolSet) ([]adktool.Tool, error) {
	...
	listByDate, err := functiontool.New(
		...
		func(toolCtx adktool.Context, in listByDateArgs) (listOutput, error) {
			entries, err := ts.ListByDate(toolCtx, agent.ListByDateArgs{
				Date: in.Date,
			})
			if err != nil {
				return listOutput{}, err
	...
	listToday, err := functiontool.New(
		...
		func(toolCtx adktool.Context, _ listTodayArgs) (listOutput, error) {
			entries, err := ts.ListToday(toolCtx)
			if err != nil {
				return listOutput{}, err
			}
			return listOutput{Entries: toPosterumViews(entries, ts.LocationFromContext(toolCtx))}, nil
